...
public class EdgeDetector extends Component {
	...
	private BufferedImage sourceImage; // 源图片
	private BufferedImage edgeImage; // 边缘图片
	private int threshold1;
	private int threshold2;
	private int threshold;
	private int widGaussianKernel;
	private String fileName;//路径
	private String name;//名字，为了输出文件
	...
	public void process(){
		if (threshold < 0 || threshold > 255 || widGaussianKernel < 3
				|| widGaussianKernel > 40)
			try {
				throw new Exception("超出其有效范围");
			} catch (Exception e) {
				e.printStackTrace();
			}
		width = sourceImage.getWidth(); // 获取源图片的宽
		height = sourceImage.getHeight(); // 获取源图片的高
		...
		for (int i = 0; i < picsize; i++) {
			...
			edgeImage.setRGB(i % width, i / width, data[i]);
		}
	...
	private int[] image2pixels(Image image) { // 转换为像素数组
	...
	public void setSourceImage(String path) {
		...
		fileName = str.substring(str.lastIndexOf('\\') + 1, str.length());// 文件相关名字，用于在数据库中索引,
		// 通过path获得路径，保存相应的输出文件
		name= path.substring(path.lastIndexOf('\\')+1, path.lastIndexOf('.'));
		//System.out.println(name);
		BufferedImage image = null;
		try {
			image = ImageIO.read(new File(path)); // 读取源图片
...
//		String fileName = "C:\\Users\\刘开\\Desktop\\20170321151053365.jpg";
//		EdgeDetector edgeDetector = new EdgeDetector();
//		edgeDetector.setSourceImage(fileName); // 设置边缘处理的参数
//		edgeDetector.setThreshold(128);
//		edgeDetector.setWidGaussianKernel(5);
//		try {
//			edgeDetector.process(); // 进行边缘处理
//			BufferedImage edgeImage = edgeDetector.getEdgeImage(); // 得到边缘图片
//			ImageIO.write(edgeImage , "jpg", new File("C:\\Users\\刘开\\Desktop\\02444.jpg"));// 输出图像
//		} catch (Exception e) {
